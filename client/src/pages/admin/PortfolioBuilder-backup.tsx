...
import { Plus, X, Sparkles, Loader2, Edit, ArrowUp, ArrowDown, Trash2, ChevronDown, Send, Wand2, Code2, Eye, EyeOff } from "lucide-react";
...
export default function PortfolioBuilder() {
  ...
  // State for Dev Mode toggle (saved in localStorage)
          ...
          <div className="flex flex-col flex-1">
            ...
            <main className="flex-1 overflow-hidden">
              ...
              <div className={`h-full ${isMobile ? 'flex flex-col' : 'flex'}`}>
                {/* Main Chat Interface - 70% width on desktop, full width on mobile */}
                <div className={`${isMobile ? 'flex-1' : devMode ? 'w-[40%]' : 'w-[70%]'} border-r flex flex-col`}>
              ...
              <Card className="p-4 space-y-4">
                ...
                {sceneType === "image" && (
                  <div className="space-y-2">
                    <Label>Image</Label>
                    <MediaSelector
                      currentValue={form.watch("url")}
                      onSelect={(url, metadata) => {
                        form.setValue("url", url);
                        // Store metadata if needed for attribution
                        if (metadata) {
                          form.setValue("media", url); // Store in media field as well
                        }
                      }}
                ...
                {sceneType === "video" && (
                  <div className="space-y-2">
                    ...
                    <MediaSelector
                      ...
                      onClear={() => form.setValue("url", "")}
                      placeholder="Enter video URL (YouTube, Vimeo, or direct URL)"
                    />
                  </div>
                )}
                ...
                {sceneType === "gallery" && (
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {/* For simplicity, allow adding multiple images one by one */}
                      <MediaSelector
                        onSelect={(url, metadata) => {
                      ...
                      {form.watch("images") && (
                        <div className="text-sm text-muted-foreground">
                          {form.watch("images").split(",").length} images added
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {sceneType === "fullscreen" && (
                  <div className="space-y-2">
                    ...
                    <MediaSelector
                      ...
                      onSelect={(url, metadata) => {
                        form.setValue("url", url);
                        // Auto-detect if it's a video URL
