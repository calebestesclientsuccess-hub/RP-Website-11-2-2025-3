...
export const PRODUCTION_AI_PROMPTS = [
  {
...
6. Immersive Entry:
   ...
   - entryEasing: "power3" or "power4" (dramatic deceleration)
...
export async function seedProductionAIPrompts() {
  console.log("🎨 Seeding production AI prompts...");
  
  let created = 0;
  let updated = 0;
  let skipped = 0;

  for (const prompt of PRODUCTION_AI_PROMPTS) {
    const existing = await db.query.aiPromptTemplates.findFirst({
      where: eq(aiPromptTemplates.promptKey, prompt.promptKey),
    });

    if (existing) {
      // Update if version is newer or force update
      if (existing.version < prompt.version) {
        await db.update(aiPromptTemplates)
          ...
          .where(eq(aiPromptTemplates.id, existing.id));
        console.log(`  ⬆️  Updated: ${prompt.promptName} (v${existing.version} → v${prompt.version})`);
        updated++;
      } else {
        console.log(`  ⏭️  Skipped (current): ${prompt.promptName} (v${existing.version})`);
        skipped++;
      }
    } else {
      await db.insert(aiPromptTemplates).values({
        ...prompt,
        id: `${prompt.promptKey}-v${prompt.version}`,
      });
