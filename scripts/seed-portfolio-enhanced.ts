...
async function seedEnhancedPortfolio() {
  ...
  try {
    ...
    // Scene 2: The Problem (Fast, Urgent)
    await db.insert(projectScenes).values({
      projectId: project.id,
      sceneConfig: {
        type: "split",
    ...
    // Scene 3: Visual Break - Team Working
    await db.insert(projectScenes).values({
      projectId: project.id,
      sceneConfig: {
        type: "image",
    ...
    // Scene 4: The Solution (Confident, Methodical)
    await db.insert(projectScenes).values({
      projectId: project.id,
      sceneConfig: {
        type: "text",
    ...
    // Scene 5: Gallery - System Components
    await db.insert(projectScenes).values({
      projectId: project.id,
      sceneConfig: {
        type: "gallery",
    ...
    // Scene 6: Social Proof (Slow, Emotional)
    await db.insert(projectScenes).values({
      projectId: project.id,
      sceneConfig: {
        type: "quote",
    ...
    // Scene 7: Results (Fast, Energetic)
    await db.insert(projectScenes).values({
      projectId: project.id,
      sceneConfig: {
        type: "image",
    ...
    // Scene 8: Final Results (Slow, Clear)
    await db.insert(projectScenes).values({
      projectId: project.id,
      sceneConfig: {
        type: "text",
    ...
    console.log("✅ Created 8 scenes with detailed director configurations");
    console.log("\n🎬 Director Flow:");
